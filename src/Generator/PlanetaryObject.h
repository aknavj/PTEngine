#ifndef __PLANETARY_OBJECT_H__
#define __PLANETARY_OBJECT_H__
	...
	public:
		...
		void Init(int nSeed, int nCraters, bool bCreate=true) {
			...
			for(i=0; i<nCraters; i++) {
				if((float)rand()/(float)RAND_MAX < 0.5f) m_nCraters--;
			}

			for(i=0; i<m_nCraters; i++) m_pCrater[i].Init();
		}
