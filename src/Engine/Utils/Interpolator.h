	...
	public:
		...
		virtual void Update() {
			...
			for(std::list<CInterpolator::Ref>::iterator i = m_list.begin(); i != m_list.end();) {
				if((*i)->Update(fSeconds)) i++;
				else i = m_list.erase(i);
			}
		}
...
/*
 */
template <class T> class TTimeInterpolator : public CInterpolator {
	...
	public:
		...
		virtual bool Update(float dt) {
			if(!m_bPaused) {
				...
				if(m_fElapsedTime >= m_fTotalTime) return false;
			}
			return true;
		}
...
class CLinearInterpolator : public TTimeInterpolator<CLinearInterpolator> {
	...
	protected:
		...
		virtual void Calculate() {
			float fRatio = MathLib::Clamp(0.0f, 1.0f, m_fElapsedTime / m_fTotalTime);
...
protected:
	...
	virtual void Calculate() {
		float fRatio = MathLib::Clamp(0.0f, 1.0f, m_fElapsedTime / m_fTotalTime);
		fRatio = MathLib::Smoothstep(0.0f, 1.0f, fRatio);
		*m_pTarget = m_fStart * (1-fRatio) + m_fEnd * fRatio;
	}
};
